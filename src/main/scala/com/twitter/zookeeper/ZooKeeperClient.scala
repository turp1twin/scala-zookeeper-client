...
class ZooKeeperClient(
  servers: String, sessionTimeout: Int, basePath : String,
  ...
  def this(servers: String, watcher: ZooKeeperClient => Unit) = this(servers, 3000, "", Some(watcher))
  def this(config: ZookeeperClientConfig, watcher: Option[ZooKeeperClient => Unit]) =
    this(config.hostList, config.sessionTimeout, config.basePath, watcher)

  def this(config: ZookeeperClientConfig) = this(config, None)
  ...
  def sessionEvent(assignLatch: CountDownLatch, connectionLatch : CountDownLatch, event : WatchedEvent) {
    ...
    event.getState match {
      ...
      case KeeperState.Expired => {
        ...
        log.info("Session was expired... creating a new zookeeper connection")
        connect()
      }
  ...
  def getChildren(path: String): Seq[String] = zk.getChildren(makeNodePath(path), false)
  def close() = zk.close
  def isAlive: Boolean = {
  ...
  def get(path: String): Array[Byte] = zk.getData(makeNodePath(path), false, null)
  def set(path: String, data: Array[Byte]) = zk.setData(makeNodePath(path), data, -1)
  def delete(path: String) = zk.delete(makeNodePath(path), -1)
